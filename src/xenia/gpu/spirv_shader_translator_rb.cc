...
void SpirvShaderTranslator::FSI_DepthStencilTest(
  ...
  for (uint32_t i = 0; i < 4; ++i) {
    ...
    // host position the coverage came from. Fragment 0 depth resolves come
    // back through restores and get retested, so the slot has to hold the
    // depth where the guest expects it.
