    ...
    if (instr.opcode == ucode::FetchOpcode::kGetTextureWeights) {
      ...
      if (instr.opcode == ucode::FetchOpcode::kGetTextureComputedLod) {
        ...
        if (use_lod_bias) {
          ...
          // host's implicit gradients - approximate them by adding the greater
          // of the two to the LOD bias. Scaling both gradients by 2^n shifts
          // the computed LOD by n, so this is exact whenever both biases are
          // equal (notably in the common all-zero case), and errs towards a
          // blurrier mip rather than a shimmering one otherwise.
