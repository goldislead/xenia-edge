  ...
  if (instr.opcode == ucode::FetchOpcode::kGetTextureGradients) {
    ...
    if (instr.opcode == ucode::FetchOpcode::kGetTextureWeights) {
      ...
      if (instr.opcode == ucode::FetchOpcode::kGetTextureComputedLod) {
        ...
        // Signs/gamma have already been applied. If the guest asks for integer
        // num_format on a fixed format, the host gave us normalized values, so
        // put them back in the guest integer range here.
        id_vector_temp_.clear();
        id_vector_temp_.push_back(
        ...
        spv::Id integer_scale_bits_packed = builder_->createLoad(
            ...
            spv::NoPrecision);
        {
          // Most fetches don't need integer scaling, so skip the work unless an
          // integer fixed fetch asked for it.
