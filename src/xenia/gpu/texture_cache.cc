...
void TextureCache::BindingInfoFromFetchConstant(
  ...
  if (fetch.dimension == xenos::DataDimension::k1D) {
    ...
    if (width_minus_1 >= xenos::kTexture2DCubeMaxWidthHeight) {
      ...
      // (and the 9-bit field couldn't even express the line width). The
      // 8192-texel rows are Xenia's own reinterpretation of contiguous linear
      // 1D data, so their stride is by construction exactly one row of texels -
      // override the pitch so the guest layout slices the data correctly
      // regardless of what the fetch constant contains.
