...
struct GetViewportInfoArgs {
  ...
  void SetupRegisterValues(const RegisterFile& regs,
    ...
    if (window_offset_in_edram_bases) {
      // The offset is in the EDRAM bases, the geometry goes through the
      // offset-0 transform. Store the effective offset so the viewport cache
      // keys on what's applied.
...
// sends both passes through the same transform. PsParamGen gets the offset
// added back in the shader. Bases going negative wrap into the previous EDRAM
// period. A host render target only holds one period past its base,
// host_render_targets_used declines draws whose unoffset rows reach past it.
