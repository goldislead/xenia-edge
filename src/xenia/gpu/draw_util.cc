...
int32_t GetWindowOffsetEdramBaseBiasTiles(
  ...
  // Every surface of the draw has to stay within the addressing period. A
  // base going negative (a per-tile depth restore into base 0, for one) is a
  // tile-local pass rather than a persistent allocation, the geometric offset
  // is the right representation for it since its depth and color draws all
