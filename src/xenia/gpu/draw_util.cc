...
int32_t GetWindowOffsetEdramBaseBiasTiles(
  ...
  // way. Declining would only send the draw back through the offset
  // viewport.
  if (host_render_targets_used) {
    // A host render target holds one EDRAM addressing period past its base,
    // tile row kEdramTileCount / pitch is the base row again (at another
    // column phase unless the pitch divides the tile count), with nothing to
    // rasterize rows past it into. A wrapped base can put the unoffset rows
