...
void DxbcShaderTranslator::RTV_AddMSAASamplesToZPDTotal(
  ...
  a_.OpIf(true, temp_z_src);
  {
    ...
    // Drivers might handle sample-frequency invocations differently.
    // Some pass the full coverage mask, others just a single bit. Since we only
    // want to count first covered sample, both approaches add coverage once.
    if (IsSampleRate()) {
