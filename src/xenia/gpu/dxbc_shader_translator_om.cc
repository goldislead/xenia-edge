...
void DxbcShaderTranslator::CompletePixelShader_DSV_DepthTo24Bit() {
  ...
  if (!DSV_IsWritingFloat24Depth()) {
    if (shader_writes_depth) {
      ...
      // Host fixed function depth bias is fine for most draws, but projected
      // decals with small bias values (4D5307E6, 4D5307E8, 4D5307FA) are
      // depth-tested redraws where matching Xenos's post viewport formula in
      // shader is more stable.
      uint32_t temp = PushSystemTemp();
      dxbc::Dest temp_x_dest(dxbc::Dest::R(temp, 0b0001));
      dxbc::Src temp_x_src(dxbc::Src::R(temp, dxbc::Src::kXXXX));
      dxbc::Dest temp_y_dest(dxbc::Dest::R(temp, 0b0010));
      ...
      a_.OpMAd(
          ...
          LoadSystemConstant(SystemConstants::Index::kEdramPolyOffsetBack,
                             ...
                             dxbc::Src::kYYYY));
      a_.OpEndIf();
      a_.OpAdd(dxbc::Dest::ODepth(), temp_x_src, in_position_z);
      PopSystemTemp();
    }
    return;
  }
    ...
    if (apply_polygon_offset) {
      ...
      a_.OpMAd(
          ...
          LoadSystemConstant(SystemConstants::Index::kEdramPolyOffsetBack,
                             ...
                             dxbc::Src::kYYYY));
      a_.OpEndIf();
      a_.OpAdd(temp_x_dest, temp_x_src, in_position_z);
